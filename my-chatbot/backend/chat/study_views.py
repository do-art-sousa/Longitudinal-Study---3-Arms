...
def study_session_complete(request):
    ...
    gidx = global_session_index(ss.week_index, ss.slot_index)
    if rcq_required_for_global_index(gidx):
        if not comprehension_provided(comprehension):
            return JsonResponse(
                {
                    "error": (
                        "comprehension responses required for this session "
                        f"(global session {gidx})"
                    )
                },
                status=400,
            )
    ...
    if participant.condition == Participant.Condition.CONTROL:
        if not validate_req_scores(req_scores):
            return JsonResponse(
                {
                    "error": (
                        "req_scores required for control arm: eight integers 1–5 "
                        "(social_presence, connection, responsiveness, autonomy, "
                        "motivation, latent_demand, unmet_need, isolation)"
                    )
                },
                status=400,
            )
    ...
    if rcq_required_for_global_index(gidx):
        ...
        from .rcq_evaluator import evaluate_rcq

        ss.rcq_score = evaluate_rcq(gidx, comprehension)

    if participant.condition == Participant.Condition.CONTROL:
        ss.req_scores = req_scores
    else:
        ss.req_scores = None

    # Store CAIQ-PANAS responses and scores if provided
    if caiq_panas:
        ss.caiq_panas_responses = caiq_panas
        # Extract scores if they were calculated on the frontend
