"""
Study arm configuration: personalized, generic, and control profiles.
...
def resolve_enrollment_code(code: str) -> Optional[str]:
    ...
    if normalized in _codes_from_env("STUDY_CODES_CONTROL"):
        return Participant.Condition.CONTROL
    return None
...
def get_profile(condition: str) -> StudyProfile:
    ...
    if condition == Participant.Condition.CONTROL:
        return StudyProfile(
            max_session_wall_minutes=int(
                getattr(settings, "STUDY_PROFILE_CONTROL_MAX_SESSION_MINUTES", 20)
            ),
            memory_enabled=False,
            allow_character_selection=False,
            default_character=str(
                getattr(settings, "STUDY_PROFILE_CONTROL_DEFAULT_CHARACTER", "default")
            ),
        )
    return StudyProfile(
        max_session_wall_minutes=int(
